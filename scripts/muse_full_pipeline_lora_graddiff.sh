...
# Full Pipeline Script for MUSE Benchmark with LoRA - GradDiff Method
...
echo "=========================================="
echo "MUSE Full Pipeline (LoRA) - GradDiff - ${DATA_SPLIT}"
echo "=========================================="
echo "Model: $MODEL"
...
if python -c "import matplotlib" 2>/dev/null; then
    ...
    python -m plot compare \
        ...
        -t "MUSE: Fine-tune vs ${TRAINER} Unlearning" 2>/dev/null || echo "⚠️  Comparison plot generation failed (non-critical)"
    ...
    python -m plot metrics \
        ...
        -t "MUSE: Metrics Comparison - ${TRAINER}" 2>/dev/null || echo "⚠️  Metrics comparison generation failed (non-critical)"
    ...
    if [ -d "../saves/finetune/${RETAIN_TASK_NAME}" ]; then
        ...
        python -m plot dashboard \
            ...
            -t "MUSE: Unlearning Dashboard - ${TRAINER}" 2>/dev/null || echo "⚠️  Dashboard generation failed (non-critical)"
    fi
